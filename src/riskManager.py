...
class Manager:
    ...
    def manage(self) -> None:
        ...
        # GET RATIO
        ratio_long = (
            self.long.amount_position * price / (self.long.amount_collateral / 10**6)
        )
        ratio_short = (
            self.short.amount_position * price / (self.short.amount_collateral / 10**6)
        )
        ...
        match (
            ...
            case (True, False):  # Long position needs collateral
                amount = int(((ratio_long / self.target_ratio_long) - 1) * 10**6)
                self.update_collat(self.long, self.short, amount)
            case (False, True):  # Short position needs collateral
                amount = int(((ratio_short / self.target_ratio_short) - 1) * 10**6)
